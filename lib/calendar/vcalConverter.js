import { convertUTCDateTimeToZone, convertZonedDateTimeToUTC, toLocalDate } from '../date/timezone';
 ...
 * @return {{date: *, tzid: *}}
 */
export const propertyToLocalDate = ({ value, parameters = {} }, tzid) => {
    if (value.isUTC) {
    ...
    // For dates with a timezone, first convert the relative date time to UTC time
    const utcTimeDate = convertZonedDateTimeToUTC(value, parameters.tzid);
    // Then convert the UTC time to the calendar time
    return toLocalDate(convertUTCDateTimeToZone(utcTimeDate, tzid));
};
