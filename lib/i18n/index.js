import { addLocale, useLocale, setDefaultLang, c } from 'ttag';
...
export async function loadLocale({ TRANSLATIONS = [] } = {}, lang) {
    const { locale, browser, language } = getLocale(lang, TRANSLATIONS);

    CACHE.translationLocale = locale;
    CACHE.browserLocale = browser;
    document.documentElement.lang = language;

    console.log({ locale, browser, language, lang });
    ...
    try {
        const rep = await fetch(`/settings/i18n/${locale}.json`);
