...
export const switchPlan = ({ planIDs, plans, planID, service, organization }) => {
    ...
    if (plansMap[PLUS].ID === planID) {
        return {
            ...
            [plansMap[DOMAIN].ID]:
                ((planIDs[plansMap[DOMAIN].ID] || plansMap[PROFESSIONAL].MaxDomains) -
                    plansMap[PROFESSIONAL].MaxDomains >
                MAX_DOMAIN_PLUS_ADDON
                    ? MAX_DOMAIN_PLUS_ADDON - 1
    ...
    if (plansMap[PROFESSIONAL].ID === planID) {
        return {
            ...
            [plansMap[DOMAIN].ID]:
                (planIDs[plansMap[DOMAIN].ID] || plansMap[PLUS].MaxDomains) - plansMap[PLUS].MaxDomains ||
                getAddonQuantity(selectedPlan, UsedDomains, 'MaxDomains', plansMap[DOMAIN]) ||
                0,
            [planID]: 1
        };
