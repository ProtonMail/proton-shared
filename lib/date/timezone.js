...
/**
 * Given a tzid, return true if it's a deprecated timezone (i.e. it's a key in the unsupportedTimezoneLinks object above)
 * @param {String} tzid
 * @return {Boolean}
 */
export const isNotSupportedTimezone = (tzid) => {
    return !!unsupportedTimezoneLinks[tzid];
};
...
export const getTimezone = () => {
    ...
    if (isNotSupportedTimezone(timezone)) {
        return unsupportedTimezoneLinks[timezone];
    }
    if (!timezone) {
...
export const getTimeZoneOptions = (date = new Date()) => {
    return (
        listTimeZones()
            ...
            .concat('UTC')
            .filter((name) => !isNotSupportedTimezone(name))
            .map((name) => {
